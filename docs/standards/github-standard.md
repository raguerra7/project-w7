...
Commits follow [Conventional Commits](https://www.conventionalcommits.org/): `type(scope): summary`, matching the [Engineering Standard](engineering-standard.md). The commit body explains *why*, not merely *what* — the diff already shows what changed.
...
- Changes to `docs/foundations/`, `docs/identity/`, `docs/framework/` or `docs/architecture/` are called out explicitly in the description, since they affect every initiative that inherits from them.
